...
def _validate_nonce(
    ...
    校验规则：
    1. 必须提供 X-Nonce 和 X-Nonce-Timestamp。
    2. 时间戳偏差不能超过 NONCE_EXPIRY_SECONDS。
    3. Nonce 在数据库中必须唯一。
    """
    ...
    now = time.time()
    if abs(now - nonce_ts) > NONCE_EXPIRY_SECONDS:
        _err(
            status_code=status.HTTP_400_BAD_REQUEST,
            code="NONCE_EXPIRED",
    ...
    # 尝试在数据库中记录（利用唯一约束）
    success = db.nonces.use_nonce(nonce, user_uuid, nonce_ts)
    if not success:
        _err(
...
async def post_chat_message(
    ...
    # 防重放校验
    _validate_nonce(x_nonce, x_nonce_ts, user_uuid, request_id)

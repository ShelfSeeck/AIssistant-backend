...
def setup_database(db_path="project.db"):
    ...
    项目/会话/消息统一使用机器时间戳字段 timestamp（float）
    """
...
def touch_session_timestamp(sid: str, db_path: str = "project.db") -> bool:
    """刷新会话 timestamp（每次聊天请求结束时调用）。"""
    now_ts = _now_timestamp()
    with db_cursor(db_path) as cursor:
        cursor.execute(
...
def create_message(
    ...
    message_timestamp = _now_timestamp()
    with db_cursor(db_path) as cursor:
        cursor.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (msg_id, sid, kind, raw_json, message_timestamp, parent_msg_id, version, is_latest, message_timestamp),
        )
